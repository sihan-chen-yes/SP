...
class AvatarTrainer:
    ...
    def forward_one_pass(self, items):
        ...
        if self.loss_weight.get('opaque', 0.) and 'mask_map' in render_output:
            rendered_mask = render_output['mask_map'].squeeze(-1) * boundary_mask_img
            # encouraging final ray opacity to be 0 or 1
            opaque_loss = binary_cross_entropy(rendered_mask, rendered_mask)
        ...
        if self.iter_idx >= self.loss_range["inverse_opacity_map"]["start"]:
            ...
            if self.loss_weight.get('depth_smooth', 0.) and 'predicted_mask' in render_output and 'predicted_depth_map' in render_output:
                predicted_mask = render_output['predicted_mask']
                predicted_depth_map = render_output['predicted_depth_map']
                depth_smooth_loss = depth_map_smooth_loss(predicted_depth_map, predicted_mask)
                # using inverse opacity map to supervise
                total_loss += self.loss_weight.get('depth_smooth', 0.) * depth_smooth_loss
                batch_losses.update({
            ...
            for batch_idx, items in enumerate(dataloader):
                ...
                if self.iter_idx % smooth_interval == 0:
                    ...
                    for key in smooth_losses.keys():
                        ...
                        wandb_loss.update(
                            {
                                'pretrain_loss/loss_' + k: v for k, v in smooth_losses.items()
                            }
                        )
                        smooth_losses[key] = 0.
                    smooth_losses = {}
                ...
                with torch.no_grad():
                    ...
                    if self.iter_idx % smooth_interval == 0:
                        ...
                        for key in smooth_losses.keys():
                            ...
                            log_info = log_info + ('%s: %f, ' % (key, smooth_losses[key]))
                            wandb_loss.update({
                                'train_loss/loss_' + k: v for k, v in smooth_losses.items()
                            })
                            smooth_losses[key] = 0.
                        smooth_losses = {}
    ...
    def mini_test(self, pretraining = False, eval_cano_pts = False):
        ...
        os.makedirs(output_dir, exist_ok = True)
        cv.imwrite(output_dir + '/iter_%d.jpg' % self.iter_idx, rgb_map)
        wandb_imgs.append(wandb.Image(rgb_map[None], caption='pretrain/test/iter_%d' % self.iter_idx))
        if eval_cano_pts:
            os.makedirs(output_dir + '/cano_pts', exist_ok = True)
        ...
        # wandb
        wandb_imgs.append(wandb.Image(predicted_mask_map[None], caption='pretrain/predicted_mask_iter_%d' % self.iter_idx))
        wandb_imgs.append(wandb.Image(gs_render["inverse_opacity_map"].cpu().numpy()[None], caption='pretrain/inverse_opacity_map_iter_%d' % self.iter_idx))

        wandb.log({self.opt["exp"] + "_images": wandb_imgs}, step=self.iter_idx)
        wandb_imgs.clear()

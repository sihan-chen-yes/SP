...
class AvatarTrainer:
    ...
    def mini_test(self, pretraining = False, eval_cano_pts = False):
        ...
        # wandb
        wandb_imgs.update({'predicted_mask': wandb.Image(predicted_mask_map)})
        wandb_imgs.update({'predicted_depth_map': wandb.Image(colored_depth_map)})
        wandb_imgs.update({'inverse_opacity_map': wandb.Image(gs_render["inverse_opacity_map"].cpu().numpy())})
        wandb_imgs.update({'inverse_depth_map': wandb.Image(colored_inverse_depth_map)})

    ...
    def compute_pca(self, n_components = 10):
        ...
        if not os.path.exists(self.data_dir + "/smpl_pos_map_{}_rot/pca_{}.ckpt".format(self.map_size, n_components)):
            ...
            for pose_idx in tqdm(self.pose_list, desc = 'Loading position maps...'):
                pose_map = cv.imread(self.data_dir + "/smpl_pos_map_{}_rot/{:08d}.exr".format(self.map_size, pose_idx), cv.IMREAD_UNCHANGED)
            ...
            pose_map = cv.imread(sorted(glob.glob(self.data_dir + '/smpl_pos_map_{}_rot/0*.exr'.format(self.map_size)))[0], cv.IMREAD_UNCHANGED)
            pose_map = pose_map[:, :pose_map.shape[1] // 2]
